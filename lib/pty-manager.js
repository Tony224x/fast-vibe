...
class PtyManager {
  ...
  spawn(index, cwd) {
    ...
    setTimeout(() => {
      ...
      if (index === 0) {
        // Pilot: disable Agent tool (force using external workers via API)
        // + inject orchestrator system prompt
        const promptPath = PILOT_PROMPT_FILE.replace(/\\/g, '/');
        cmd = `claude --dangerously-skip-permissions --disallowedTools Agent --append-system-prompt-file "${promptPath}"`;
      }
