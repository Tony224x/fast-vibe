...
class PtyManager {
  ...
  _processSuggestQueue() {
    ...
    sg.pty.write(message);
    setTimeout(() => {
      if (sg.pty) sg.pty.write('\r');
    }, 150);
    ...
    const pollInterval = setInterval(() => {
      ...
      if (suggestMatch) {
        ...
        if (text && !text.startsWith('NONE')) {
          this.suggestions[workerId] = { text, source: 'ai', pending: false };
      ...
      // Wait for enough output and an idle prompt marker
      if (stripped.length > 200 && /❯\s*$/.test(stripped)) {
        ...
        if (lastLine && lastLine.length > 1 && lastLine.length < 300) {
          ...
          this.suggestions[workerId] = { text: cleaned, source: 'ai', pending: false };
        } else if (this.suggestions[workerId]) {
          this.suggestions[workerId].pending = false;
        }
        this.suggestBusy = false;
